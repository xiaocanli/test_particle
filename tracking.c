...
void GatherParticleSpectra(int num_threads, double espectrum[][nbins],
        double espect_private[][nbins*nt_out], double espect_escape[][nbins],  
        double espect_escape_private[][nbins*nt_out]);
void MultiStepsParticleTrackingOmp(int nptl, double dt,
        int tid, int sstep, int estep,
        int einterval_t, int traj_diagnose, struct particles *ptl,
        int *nsteps_ptl_tracking, double espect_private[][nbins*nt_out], 
        double espect_escape_private[][nbins*nt_out]);
void SingleStepParticleTrackingOMP(int iptl, int it, int *ntp, double dt,
        int tid, int einterval_t, int traj_diagnose,
        struct particles *ptl, int *iescape_pre, int *iescape_after, 
        int ntraj_offset_local, int *ntraj_diagnostics_points, int *nsteps_ptl_tracking,
        ...
        double espect_escape_private[][nbins*nt_out])
{
    ...
    for (current_slice = 1; current_slice < simul_grid.nt; current_slice++) {
        ...
        starting = ending + 1;
        ending = floor(((current_slice+1.0)*vfield_dt)/dt);
        /* Waiting for all threads to reach this point. */
        #pragma omp barrier
        /* Only one thread is going to update the velocity field. */
        #pragma omp master
        ...
        double espect_escape_private[][nbins*nt_out])
{
    ...
    for (i=0; i<nptl; i++) {
        ...
        for (j=sstep; j<=estep; j++) {
            SingleStepParticleTrackingOMP(i, j, &ntp, dt,
                tid, einterval_t, traj_diagnose,
                ptl, &iescape_pre, &iescape_after, 
                ntraj_offset_local, &ntraj_diagnostics_points, nsteps_ptl_tracking,
                espect_private, espect_escape_private);
            if (traj_diagnose == 1) {
        ...
        double espect_escape_private[][nbins*nt_out])
{
    ...
    if (*iescape_pre == 0 && *iescape_after == 1 && bc_flag == 1) {
        ...
        ptl_energy_fixed(iptl, *ntp, tid, ptl, espect_escape_private);
    }
    int tmp;
    tmp = *iescape_after;
    *iescape_pre = tmp; /* Update particle status. */
    if (it % einterval_t == 0) {
        if (*iescape_after == 0) {
            ...
            ptl_energy_fixed(iptl, *ntp, tid, ptl, espect_private);
        }
        (*ntp)++;
    }
}
...
//        printf("tid %d\n", tid);
        ...
        for (i=0; i<nptl; i++) {
            int iescape_pre = 0;
            int iescape_after = 0;
            int tp_pre, tp_after = 1; /* Tracking dense output points. */
            ...
            while (t < simul_domain.tmax) {
                ...
                if (iescape_pre == 0 || bc_flag == 0) {
                    /* Check if the particle already exit the box.
                     * Using Bulirsch-Stoer ODE Integrator. */
                ...
                iescape_pre = iescape_after; /* Update particle status. */
                if (tp_after > tp_pre && iescape_after == 0) {
                    /* It passes dense output points, so dense output is needed. */
                    ptl_energy_adaptive(ydense, tp_after, 
 ...
 *  nptl: particle number for this process.
 *  dt: time step for particle tracking.
 *  traj_diagnose: flag for trajectory diagnostics. 
 * Output:
        ...
        int traj_diagnose, struct particles *ptl)
{
    ...
    double espect_escape[nt_out][nbins]; /* Energy spectrum for 
                                          * escaping particles.*/
    init_spectrum(espectrum);
    init_spectrum(espect_tot);
    init_spectrum(espect_escape);
    ...
    if (iadaptive == 0) {
        particle_tracking_fixed(nptl, dt, 
                traj_diagnose, ptl, espectrum, 
                espect_tot, espect_escape,
                espect_private, espect_escape_private);
    }
    else if (iadaptive == 1) {
        particle_tracking_adaptive(nptl, 
                traj_diagnose, ptl, espectrum, 
                espect_tot, espect_escape,
                espect_private, espect_escape_private);
    }
    ...
    if (traj_diagnose == 0) {
        espectrum_collect(espectrum, espect_tot, "espectrum.dat");
        if (bc_flag == 1) {
            /* Only for open boundary conditions. */
            espectrum_collect(espect_escape, espect_tot, "espect_escape.dat");
        }
    }
}
