...
/* int iadaptive; /1* Tracking method is adaptive or nor. *1/ */
/* int nt_out=100; /1* Number of spectra outputs. *1/ */
...
/*         /1* Check if the particle already exit the box. *1/ */
/*         //advance(&ptl[i], dt); */
...
/*                 /1* Save the diagnostic particles in aligned memory. *1/ */
...
/*     int tmp; */
/*     tmp = *iescape_after; */
/*     *iescape_pre = tmp; /1* Update particle status. *1/ */
/*     if (it % einterval_t == 0) { */
/*         if (*iescape_after == 0) { */
/*             /1* Particles haven't escaped from simulation box. *1/ */
        ...
        struct particles *ptl)
{
    ...
    if (tracking_method == 0) {
        /* particle_tracking_fixed(nptl, dt, traj_diagnose, ptl, espectrum, */ 
