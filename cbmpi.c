 ...
 *  isystem is read in this function (0: test case, 1: wlcs, 2: force-free);
 ******************************************************************************/
void read_domain()
{
    FILE *fp;
    ...
    /* Special information for each system */
    if (isystem == 1) {
        read_wlcs();
    }
    else if (isystem == 2){
    ...
    /* Print the corresponding system information. */
    if (my_id == 0) {
        if (isystem == 0) {
